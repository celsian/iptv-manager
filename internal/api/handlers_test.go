...
func TestHandleGetChannelURLFromCache(t *testing.T) {
	server, tmpDir := setupTestServer(t)

	// Create a cached playlist with a channel
	playlistDir := filepath.Join(tmpDir, "playlists")
	m3uContent := `#EXTM3U
#EXTINF:-1 tvg-id="123" tvg-name="Test Channel",Test Channel
http://example.com/stream/123`
	os.WriteFile(filepath.Join(playlistDir, "Test.m3u"), []byte(m3uContent), 0644)

	req := httptest.NewRequest("GET", "/api/playlists/channel-url?channelId=123&playlist=Test", nil)
	w := httptest.NewRecorder()

	server.handleGetChannelURL(w, req)
	...
	if response["url"] != "http://example.com/stream/123" {
		t.Errorf("URL = %q, want %q", response["url"], "http://example.com/stream/123")
	}
}

func TestHandleGetChannelURLMissingParam(t *testing.T) {
	server, _ := setupTestServer(t)

	req := httptest.NewRequest("GET", "/api/playlists/channel-url", nil)
	w := httptest.NewRecorder()
...
func TestHandleGetChannelURLNotFound(t *testing.T) {
	server, _ := setupTestServer(t)

	// No cached playlist, no IPTV credentials configured = should fail
	req := httptest.NewRequest("GET", "/api/playlists/channel-url?channelId=nonexistent", nil)
	w := httptest.NewRecorder()
